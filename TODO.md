...
- [ ] **엔트리포인트/패키징 정합성**
- [ ] **ConfigManager 단일 진실원 정리**
- [ ] **Pipeline 공개 경계 복구**
- [ ] **WorkerPool / Session 책임 정리**
...
- [ ] `pyproject.toml` script entrypoint와 실제 모듈 경로를 일치시킨다
- [ ] GUI 진입점(`main.py`), CLI 진입점, 설치형 script가 동일한 공개 실행 경로를 사용하도록 정리한다
- [ ] `python -m ...`, 설치형 script, 로컬 GUI 실행이 모두 깨지지 않도록 테스트 기준을 맞춘다
- [ ] `setup.md`, `README.md`, `AGENTS.md`의 실행 예시를 실제 공개 진입점 기준으로 정리한다

### [A2] ConfigManager 단일 진실원 정리 `Wave A1`

- [ ] `ConfigManager.set()` 이후 typed settings(`app_settings`, `processing_settings`, `chat_settings`, `rendering_settings`) 갱신 정책을 고정한다
- [ ] raw `get()` 기반 접근과 typed settings 접근이 같은 값을 보도록 정리한다
- [ ] `SettingsDialog`를 영속 설정 편집기 역할로 고정하고, 작업 시작용 옵션과 구분한다
- [ ] 기본 플러그인/언어/`use_agent`/테마의 초기값, 저장, 재적용 경로를 일치시킨다
- [ ] 설정 UI에서 수정 가능한 값이 실제 저장 및 다음 실행에 반영되도록 테스트를 보강한다

### [A3] Pipeline 공개 경계 복구 `Wave A2`

- [ ] GUI 레이어가 `Pipeline` 내부 구현(`_export_service` 등)에 직접 접근하지 않도록 제거한다
- [ ] export/preview/reprocess 동작은 공개 메서드 또는 전용 use case를 통해서만 호출되게 정리한다
- [ ] `MainWindow`는 orchestration만 담당하고 비즈니스 동작은 controller/use case로 내린다
- [ ] 관련 단위 테스트를 공개 경계 기준으로 정리한다

### [A4] WorkerPool / Session 책임 정리 `Wave A2`

- [ ] `WorkerPool.max_concurrent`를 실제로 강제할지, 단순 워커 매니저로 축소할지 결정하고 구현을 일치시킨다
- [ ] 동시 실행 초과 시 정책을 명시한다: 대기열 적재 또는 즉시 거절
- [ ] `Session`의 상태 저장 책임과 `WorkerPool`의 실행 스케줄링 책임을 분리한다
- [ ] 단일 이미지 처리와 채팅 배치 처리의 동시 실행 허용 범위를 정리한다
- [ ] 관련 테스트를 동시성 정책 기준으로 보강한다

### [A5] 아키텍처 검증 및 문서 정리 `Wave A3`

- [ ] 엔트리포인트, 설정 반영, 공개 경계, 동시성 제한에 대한 회귀 테스트를 추가한다
- [ ] `TODO.md` 완료 시점에 실행/설정/작업 흐름 문서를 현재 구조와 일치시킨다
- [ ] 변경 후 `pytest --cov=src` 재실행으로 핵심 경로 회귀를 확인한다
...
### [B1] 항목 1 Phase 6: UX 편의 기능 `Wave B1` _(Wave A 완료 후)_
